...
class ifx_dig_sfr_test extends ifx_dig_testbase;
    ...
    function new(string name = "ifx_ifx_dig_sfr_test", uvm_component parent);
    ...
    task main_phase(uvm_phase phase);
        phase.raise_objection(this); //testul nu ruleaza fara raise_objection
        ...
        /*
        write_reg_fields(
            .reg_name("NONE"), //incercam scrierea si citirea dintr un registru care nu exista
            .fields_names({"FILTER_TYPE"}),
            .fields_values({2'b11})
        );
        */
        ...
        for (int addr=0; addr < 2**`AWIDTH; addr++) begin  //vreau sa obtin toate adresele, folosesc `AWIDTH

            `TEST_INFO($sformatf("Write all 1's to address %b", addr))
            data_bus_write_seq.address = addr;
            data_bus_write_seq.data = 8'hFF;
            data_bus_write_seq.start(dig_env.data_bus_uvc_agt.sequencer); //pornesc secventa, ii dau sequencerul pe care sa se duca drept parametru, din environment trec in agent dupa in sequencer
